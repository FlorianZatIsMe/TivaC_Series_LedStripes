...
/*! \details Pre-calculated CRC-8 table for polynomial x8 + x5 + x4 + 1 */
...
{	
	DisableInterrupts();
	//printf("\FIFO_Put: %ld", data);
	if(FIFO_front == (FIFO_back+1) % FIFOSIZE)
	{	
		FIFO_full = 1;
		EnableInterrupts();
		return 0;	// if FIFO is full return an erreur
	}

	FIFO[FIFO_back] = data;
	
	FIFO_back = (FIFO_back+1) % FIFOSIZE;
	FIFO_full = 0;
	
	//printf("\nFIFO put data: %#x\nFront: %ld - Back : %ld\n", data, FIFO_front, FIFO_back);
	EnableInterrupts();
	return 1;
}
...
{	
	...
	{
		EnableInterrupts();
		return 0;	// if FIFO is empty return an erreur
	}
	
	*(data) = FIFO[FIFO_front];
	FIFO_front = (FIFO_front+1) % FIFOSIZE;
	EnableInterrupts();
	//printf("\nFIFO get data: %#x\nFront: %ld - Back : %ld\n", test_data, getFront(), getBack());
...
{
	if(FIFO_front > FIFO_back) return (FIFO_back - FIFO_front +256);
	return (FIFO_back - FIFO_front);
}
